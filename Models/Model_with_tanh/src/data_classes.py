# Contains classes for dataset and model with tanh implementation for geometric parameters,
# such that they don't vary too much
...
class Model(nn.Module):
    """
    Creates the Model for the optimization process, with parameters divided in
    - globals, for global coefficients
    - couplings, for particle dependent couplings
    - equil, containing non-varying geometric parameters
    - amplitudes, with the parameters that appear in the tanh function, describing the divergence from equil values
    ...
    def __init__(self, top_pars=None, dat_pars=None, device='cpu'):
        ...
        eps_bonds = np.load('../data/bonds_gausspar.npy')[:,1]
        eps_angles = np.load('../data/angles_gausspar.npy')[:,1]
        eps_torsions = np.load('../data/dihedrals_gausspar.npy')[:,1]

        self.device = device
        self.couplings = self.Interactions()
        self.amplitudes = self.Interactions()
        self.equil = self.Interactions()
        self.eps = self.Interactions()
        self.globals = Parameter(torch.tensor(dat_pars, dtype=torch.float, device=self.device, requires_grad=True))
        self.couplings.bonds = Parameter(torch.tensor(top_pars['bond_type'][:12,0], dtype=torch.float, device=self.device, requires_grad=True))
        self.couplings.angles = Parameter(torch.tensor(top_pars['angle_type'][:13,0], dtype=torch.float, device=self.device, requires_grad=True))
        ...
        self.amplitudes.bonds = Parameter(torch.zeros(11, dtype=torch.float, device=self.device, requires_grad=True))
        self.amplitudes.angles = Parameter(torch.zeros(12, dtype=torch.float, device=self.device, requires_grad=True))
        self.amplitudes.torsions = Parameter(torch.zeros(25, dtype=torch.float, device=self.device, requires_grad=True))
        self.equil.bonds = Parameter(torch.tensor(top_pars['bond_type'][:12,1], dtype=torch.float, device=self.device, requires_grad=False))
        self.equil.angles = Parameter(torch.tensor(top_pars['angle_type'][:13,1], dtype=torch.float, device=self.device, requires_grad=False))
        ...
        self.eps.bonds = torch.tensor(eps_bonds, dtype=torch.float, device=self.device, requires_grad=False)
        self.eps.angles = torch.tensor(eps_angles, dtype=torch.float, device=self.device, requires_grad=False)
        self.eps.torsions = torch.tensor(eps_torsions, dtype=torch.float, device=self.device, requires_grad=False)
    ...
    def forward(self, X):
        ...
        for i in range(X_lengths.shape[0]):
            ...
            energy[i,0] = le.bonds_energy(coords, bonds, self.couplings, self.equil, self.amplitudes, self.eps, self.globals)
            energy[i,1] = le.angles_energy(coords, angles, self.couplings, self.equil, self.amplitudes, self.eps, self.globals)
            energy[i,2] = le.torsions_energy(coords, tors, self.couplings, self.equil, self.amplitudes, self.multiplicity, self.eps, self.globals)

        # print(energy)
        return energy  # .sum(dim=0).squeeze() / X_lengths.shape[0]
