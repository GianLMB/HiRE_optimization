# All the versions of classes for the loading of the Dataset and of the ML model
...
import src.LocalEnergyVct as le
import src.LocalEnergy as ley
...
class RNASeqDataset(Dataset):
    ...
    def __init__(self, device='cpu', csv_file='../data/CSV_minimized/seq_frame.csv', root_dir='../data/CSV_minimized/', transform=None):
        """
...
class LocalEnergyOpt(nn.Module):
    ...
    def forward(self,X):
        ...
        return energy.squeeze()  # .sum(dim=0).squeeze() / X_lengths.shape[0]
...
class LocalEnergy(nn.Module):
    ...
    - opt_pars, for global coefficients
    ...
    def forward(self, X):
        ...
        for i in range(X_lengths.shape[0]):
            ...
            angles = features[:lengths[7], 7].long().view(-1, 4)
            tors = features[:lengths[8], 8].long().view(-1, 5)
            energy[i, 0] = ley.bonds_energy(coords, bonds, self.couplings, self.equil, self.opt_pars)
            energy[i, 1] = ley.angles_energy(atoms, coords, angles, self.couplings, self.equil,  self.opt_pars)
            energy[i, 2] = ley.torsions_energy(atoms, coords, tors, self.couplings, self.equil,  self.multiplicity, self.opt_pars)
...
class RNASeqDataset_old(Dataset):
    """RNA sequences dataset."""
