...
def bonds_energy(coords, bonds, couplings, equil, pars):
    ...
    energy = (pars[0]*torch.abs(couplings.bonds[idx])*(d-equil.bonds[idx])**2).sum()
...
def angles_energy(atoms, coords, angles, couplings, equil, pars):
    ...
    energy = (pars[46] * pars[thetatype+1] * torch.abs(couplings.angles[idx]) * (an - equil.angles[idx])**2).sum()
...
def torsions_energy(atoms, coords, tors, couplings, equil, mult, pars):
    ...
    energy = (pars[9] * pars[28+phitype] * torch.abs(couplings.torsions[idx]) * (1+torch.cos(mult[idx]*phi - equil.torsions[idx]))).sum()
