...
setenv Simulation_Type                 serial  # Simulation type: serial or replica (exchange)
...
setenv Temperature                     150.0d0 # Tem perature in kelvins
setenv N_Frag                             1    # Number of fragments 
setenv RANDOM_SEED                        0     # set to zero or remove to use the clock

setenv Potential_Scaling_Factor          1.7   # This is use to strengthen the potential when doing MD
...
setenv COMPUTE_SAXS_SERIAL           .true.   # Compute the SAXS force
...
setenv SAXS_Omega                       0.037  # FF'_HOH=FF_HOH*omega added by lina
setenv SAXS_Norm_Type                     3    # 1 for 1-norm, 2 for 2-norm, 3 for infinity-norm
setenv SAXS_Vect_Max                      1.0  # Maximum value of the scattering vector (default and max is 1)
setenv SAXS_Norm_Range               "0.01 1.0"    # q-range on which to apply the norm calculations UNUSED
setenv SAXS_Calculation_Type          solution    # Either 'vacuum' or 'solution', depending on the type of calculation we are looking for
setenv SAXS_Refine_Hydration          .true.   # .true. for refined hydration, .false. otherwise (default)
setenv SAXS_Dummy_Water_Radius           3   # Radius of the dummy waters for Hydration algorithm (Only if SAXS_Refine_Hydration=.true.) IT IS NOT ACTUALY THE RADIUS, IT IS THE DIAMETER
...
setenv Max_Num_Iter_Minimization         2   # Maximum number of iteration (minimization) 
...
setenv n_save_configs                  10     # Save configs every n steps (default: 1000)
