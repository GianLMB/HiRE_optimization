...
contains
  ...
  subroutine tensor_product(t, v1, v2, f )
    ...
    real(8), intent(in) :: f
    real(8), dimension(3), intent(in) :: v1, v2
    real(8), dimension(3,3), intent(out) :: t
    integer :: i, j
  ...
  subroutine rotation_matrix(natoms,posa,posb,M)
    ...
    real(8), dimension(3*natoms), target :: posa,posb
    real(8), dimension(3,3)  :: cpp,cpp1,cpp2,cpp3    ! Matrix C"  
    real(8), dimension(3, 3) :: M,M1,M2,M3            ! Matrix M 
    real(8), dimension(3, 3) :: matrc                 ! Matrix C 
    real(8), parameter :: eps = 1.0E-12
    real(8) :: alpha, beta, gama, s, t, st2, st
...
!         write(*,*) "Exiting after ", maxiter, "iterations."
  ...
  subroutine minimize_rmsd(nat,posref,pos,delr,rmsd,npart,calpha_in,atomic_type_in)
    ...
    real(8), dimension(3*nat), intent(inout), target :: posref, pos
    real(8), dimension(:), allocatable, target :: posa, posb, pospb

    real(8), dimension(:), pointer :: xa, ya, za   ! Pointers for working position
    real(8), dimension(:), pointer :: xb, yb, zb   ! Pointers for reference position
    real(8), dimension(:), pointer :: xpb, ypb, zpb
    real(8), dimension(3,3) :: M
    real(8) :: dr2,dr,delr2, delx,dely,delz,delr, rmsd
    integer :: i,j,npart
  ...
  subroutine geometric_center(natoms,vector)
    implicit none
    integer :: natoms
    real(8), dimension(3*natoms), intent(inout), target :: vector
    real(8), dimension(3) :: centrd  ! centroid or center of mass     
  ...
  subroutine remove_total_momentum(natoms,vec,mass)
    implicit none
    integer, intent(in) :: natoms
    real(8), dimension(3*natoms), intent(inout), target :: vec
    real(8), dimension(3*natoms), intent(in),    target :: mass 
    real(8), dimension(:), pointer :: vx, vy, vz

    real(8) :: pxtot, pytot, pztot, mcm
...
!    vz = vz - pztot/mass(1:natoms)   
  ...
  subroutine remove_rotation(natoms,pos,vel,mass) 
    ...
    integer, intent(in) :: natoms
    real(8), dimension(3*natoms), intent(in), target    :: pos, mass
    real(8), dimension(3*natoms), intent(inout), target :: vel
    
    integer :: i
    real(8), dimension(:), pointer :: x, y, z, vx, vy, vz
    real(8) :: trace
    real(8), dimension(3)   :: l, temp, v, l1, v1, o
    real(8), dimension(3,3) :: inertia, i1
...
!    cm = cm/total_mass
   ...
   subroutine solve_3x3(U,V,W)
    implicit none
    real(8), dimension(3,3), intent(in) :: U
    real(8), dimension(3), intent(in)   :: V
    real(8), dimension(3), intent(out)  :: W

    real(8) :: a, b, c, d, e, f, p, q, o
  ...
  subroutine matrix_c(natoms,posa,posb,mtr_c)
    ...
    integer, intent(in) :: natoms
    real(8), dimension(3*natoms), intent(in), target :: posa, posb
    
    real(8), dimension(3,3), intent(out) :: mtr_c
  ...
  subroutine get_momentum_rotation(nstep,natoms,pos,vel,mass)
    ...
    real(8), dimension(3*natoms), intent(in), target :: pos, vel, mass
    real(8), dimension(:), pointer :: x, y, z, vx, vy, vz, ms
