...
extern int      f77init;             /* zero array first time */
...
void
F77_FUNC(writextc,WRITEXTC)(int *fid, int *nat, int *nstep, float *time, matrix *box, 
//						 *x, float *precision, int *ret)
						double *x, float *precision, int *ret)
//						rvec *x, float *precision, int *ret)
{
		if(*nat > mesx_len) {
      mesx_len = *nat;
      mesx = realloc(mesx, mesx_len * sizeof(rvec));
    }
		int i;
		for(i = 0; i < *nat; ++i) {
				mesx[i][0] = x[i*3];
    ...
    *ret = write_xtc(f77xdr[*fid], *nat, *nstep, *time, *box, mesx, *precision);
}
