...
module constraints
  ...
  real(8)                                     :: energy_c
  real(8), dimension(:), allocatable, target  :: fc, pc
  real(8), dimension(:), pointer              :: cx, cy, cz
  real(8), dimension(:), pointer              :: fcx, fcy, fcz
...
contains
  subroutine set_harmonic_constraints(nat,posa)
    ...
    real(8), dimension(3*nat), target, intent(in) :: posa
    ...
    ! Moved to initialise_minimization
  ...
  subroutine harmonic_constraints(nat,posa,forca,t_energy)
    ...
    integer, intent(in) :: nat
    real(8), dimension(3*nat), target, intent(in)    :: posa
    real(8), dimension(3*nat), intent(inout) :: forca
    real(8), intent(inout)                   :: t_energy
...
module md_utils
  ...
  subroutine integrate(energyscale,constrained_energy)
    real(8), intent(in) :: energyscale
  ...
  subroutine integrate_log(energyscale,constrained_energy,logener,simuPercent)
    real(8), intent(in) :: energyscale
    double precision :: constrained_energy
    logical :: logener
  ...
  !> @brief  This routine applies velocity Verlet with the possibility of rattle
...
!!!RL     endif
  ...
  !> @brief  Initialise the velocities for the MD
  subroutine initialise_velocities(temperature) 
    implicit none
    
    integer :: i
    real(8), intent(in) :: temperature
    real(8) :: rescale, current_temperature
    real(8) :: gaussian_number
  ...
  !****************************************************************************
  subroutine scale_velocities(thermal_bath, temperature)
    implicit none
    
    real(8), intent(in) :: temperature
    real(8) :: twokinetic, current_temperature, rescale
    character(len=20), intent(in) :: thermal_bath

    integer :: i, it, i1, i2, xii
    double precision :: gmaxbf, sdots, g0
    double precision :: vh2, vh2max, vn2
    double precision, dimension(3) :: qij, rij, sij
    real(8) pbc_mic
    real(8) :: c2
    real(8), external :: gaussian_number
...
!!!RL        enddo
  ...
  !****************************************************************************
  !> @brief   Forces the atoms to stay within a sphere by bouncing all atoms that cross
  !****************************************************************************
  subroutine confine_in_sphere()
    implicit none
    integer :: i
    real(8) :: r2, xx, yy, zz, inverse_r, parallel_component
...
!!!        write(*,*) 'Bouncing atom: ', i
  ...
  !****************************************************************************
  !> @brief   First step of rattle
  !****************************************************************************
  subroutine rattles_1()
    implicit none

    integer :: i,it, ic, i1, i2, xii
    real(8) :: g0, rij2, sdotr
    real(8) :: pbc_mic, RMA, RMB, GAB
    real(8), dimension(3) :: rij, DX
...
! Evolve positions
  ...
  !****************************************************************************
  !> @brief  Second step for rattle
  !****************************************************************************
  subroutine rattles_2()
    implicit none

    integer :: i,it, ic, i1, i2
    real(8) :: RMA, RMB, GAB, rdotv
    real(8) pbc_mic
    real(8), dimension(3) :: vij, DX
    real(8), dimension(:,:),  allocatable :: sij
    logical :: done
    logical, dimension (:), allocatable :: moving, moved
