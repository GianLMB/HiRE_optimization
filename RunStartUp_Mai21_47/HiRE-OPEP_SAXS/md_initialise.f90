module md_initialise
  ...
  use md_utils
  use restart_module
  use SAXS_scoring, only: initialise_SAXS
contains
  ...
  subroutine initialise(conformation)
    ...
    ! lm759 > SAXS initialization
    if (compute_saxs_serial .or. simulation_type .eq. 'saxs')  then 
  ...
  subroutine initialise_potential()
    use fragments, only: nfrag
    implicit none
    
    integer :: i
    real(8), dimension(vecsize) :: xpos    ! Positions ( (x1,y1,z1),(x2,y2,z2)...
    real(8), dimension(natoms)  :: amass   ! 1 over the masses
  
    ! lm759> Initialise nfrag from fragments
    nfrag = 0
    ! The order of initialise() calls matter here
    inquire(file="conf_initiale.pdb", exist=prot_simulation)
    if (prot_simulation) then
      call initialise_protein(PBC,BL,C_M,natoms,xpos,amass,atomic_type)
    endif
    inquire(file="conf_initiale_RNA.pdb", exist=RNA_simulation)
    if (RNA_simulation) then
      call initialise_RNA(PBC,BL,C_M,natoms,xpos,amass,atomic_type)
    endif
    if (natoms .ne. N_prot+N_RNA) then
