...
class Model(nn.Module):
    """Creates the Model for the optimization process."""
    ...
    def __init__(self, top_pars=None, dat_pars=None, device='cpu'):
        ...
        eps_bonds = np.load('bonds_gausspar.npy')[:,1]
        eps_angles = np.load('angles_gausspar.npy')[:,1]
        eps_torsions = np.load('dihedrals_gausspar.npy')[:,1]
    ...
    def forward(self, X):
        return  # .sum(dim=0).squeeze() / X_lengths.shape[0]
